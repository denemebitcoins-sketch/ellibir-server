...
-- ─────────────────────────────────────────────────────────────────────
-- BOLUM 43) MAGAZA ELMAS -> CIP TAKASI (CLIENT-CAGRILI, OTORITER)
-- Client yalniz paket kimligi yollar. Fiyat ve odul sunucuda sabittir;
-- profil satiri FOR UPDATE ile kilitlenip iki bakiye tek transaction'da yazilir.
-- ─────────────────────────────────────────────────────────────────────
...
begin
  ...
  if v_diamonds < v_diamond_cost then
    return jsonb_build_object('ok', false, 'error', 'insufficient_diamonds', 'chips', v_chips, 'diamonds', v_diamonds);
  end if;

  v_chips := v_chips + v_chip_delta;
  v_diamonds := v_diamonds - v_diamond_cost;
  update public.profiles
     set chips = v_chips, diamonds = v_diamonds, updated_at = now()
   where id::text = v_uid::text;
  return jsonb_build_object(
    'ok', true, 'error', '', 'chips_delta', v_chip_delta, 'diamonds_delta', -v_diamond_cost,
...
-- ─────────────────────────────────────────────────────────────────────
-- TEST MAĞAZASI ELMAS PAKETİ — Google Play receipt doğrulaması gelene kadar
-- online testte alınan elmasın cüzdan yenilemesinde geri düşmesini engeller.
-- Paket kimliği dışında miktar client'tan alınmaz; profil satırı atomik kilitlenir.
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- BÖLÜM 45) PROFİL SOSYAL GÖRÜNÜRLÜĞÜ (2026-07-13)
-- Açık / yalnız arkadaşlar / gizli. Temel profil görünür kalır; arkadaş
-- listesi, duvar, beğeni ve yorum okumaları RLS ile korunur.
-- ─────────────────────────────────────────────────────────────────────
...
returns table(id text, name text, gender text, role text, chips bigint, avatar_url text)
language plpgsql
stable
security definer
set search_path = public
...
begin
  ...
  select p.id::text,
    ...
    join public.profiles p
      on p.id::text = case when f.requester::text = p_user then f.addressee::text else f.requester::text end
   where f.status = 'accepted'
     and (f.requester::text = p_user or f.addressee::text = p_user)
...
drop policy if exists posts_select on public.posts;
create policy posts_select on public.posts
  for select to authenticated using (public.can_view_profile_social(user_id::text));
drop policy if exists post_likes_select on public.post_likes;
create policy post_likes_select on public.post_likes
  for select to authenticated using (public.can_view_post_social(post_id));
drop policy if exists post_likes_insert on public.post_likes;
create policy post_likes_insert on public.post_likes
  for insert to authenticated
...
create policy post_comments_insert on public.post_comments
  for insert to authenticated
  with check (auth.uid() = user_id and public.can_write_social() and public.can_view_post_social(post_id));
